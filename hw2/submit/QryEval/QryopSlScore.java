...
public class QryopSlScore extends QryopSl {
 ...
 /**
  ...
  public QryResult evaluateBM25(RetrievalModelBM25 r) throws IOException {
	    ...
	    long doclen;
	    float avg_doclen= QryEval.READER.getSumTotalTermFreq(result.invertedList.field) /
	    ...
	    for (int i = 0; i < result.invertedList.df; i++) {
	    	...
	    	doclen = dls.getDocLength(result.invertedList.field, result.invertedList.postings.get(i).docid);
	    	
	    	score= rsj* result.invertedList.postings.get(i).tf/
	    			(result.invertedList.postings.get(i).tf+r.k1*((1-r.b)+r.b*doclen/avg_doclen));
